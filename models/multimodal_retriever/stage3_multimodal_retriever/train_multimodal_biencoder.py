...
from faiss_utils import save_index, load_index
...
def make_batch_with_hard_negs(
    ...
    Args:
        examples: Pool of all examples
        hard_negs: Dictionary mapping example IDs to lists of hard negative IDs
        batch_size: Target batch size
        hard_neg_k: Number of hard negatives to include per anchor
    
    Returns:
        Batch of examples (anchors + hard negatives + random samples)
    """
...
def main():
    ...
    parser.add_argument("--max_examples", type=int, default=None, help="Max examples to use")
    
    # Model arguments
    parser.add_argument("--text_model", default="sentence-transformers/all-mpnet-base-v2",
                        help="Text encoder model name")
    parser.add_argument("--image_model", default="openai/clip-vit-base-patch32",
    ...
    parser.add_argument("--hard_mining_every", type=int, default=1,
                        help="Mine hard negatives every N epochs (0 to disable)")
    parser.add_argument("--hard_topm", type=int, default=50,
                        help="Number of top neighbors to retrieve for hard negatives")
    parser.add_argument("--hard_neg_k", type=int, default=2,
                        help="Number of hard negatives per anchor in batch")
    parser.add_argument("--query_view", default="image+question",
    ...
    parser.add_argument("--save_dir", default="multimodal_biencoder_checkpoints",
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.hard_mining_every > 0 and (epoch == 1 or epoch % args.hard_mining_every == 0):
            ...
            print(f"  Mining hard negatives (query_view={args.query_view}, top-{args.hard_topm})...")
            ...
            save_index(index, index_path, metadata={
                "epoch": epoch,
        ...
        if epoch % args.save_every == 0:
            ...
            save_checkpoint(model, optimizer, epoch, global_step, avg_epoch_loss, checkpoint_path)
